        ...
        with scope("Sampling"):
            for idx, (image_path, targets_path, mask_path) in tqdm.tqdm(enumerate(zip(images, targets, masks)), total = len(masks), desc="Producing images triplets"):
                ...
                outpath = os.path.join(
                    opt.outdir,
                    f'{opt.lpips}_{os.path.split(os.path.basename(image_path))[1].split(".")[0]}_{str(round(lpips_value, 4))}_{opt.prefix}_{ema_prefix}_{os.path.basename(opt.ckpt).split(".")[0]}.png',
                )
