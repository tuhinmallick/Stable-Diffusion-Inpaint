...
def from_mask_to_seg(path):
    return os.path.dirname(path) + "/" + os.path.basename(path).replace("mask", "seg") 


df = pd.DataFrame(columns=["image_path","mask_path","segmask_path","partition"])

modality_dir = "interiornet_mask_generated/generated_with_seg/"
validation_percentage = 0.2

for folder in glob.glob(modality_dir + "masked_views_*"):
    print(folder)
    mother_dir = "%s%s/" % (modality_dir,os.path.basename(folder))
    ...
    list_samples_image, list_samples_mask,list_samples_seg, list_partition = [],[],[],[]
    ...
    for i in range(0, len(masked_images), sampl_validation_rate):
        ...
        list_samples_seg.extend([from_mask_to_seg(x).replace(prefix_to_remove, "") for x in samples])

        list_partition.extend(["train"]*(len(samples)-how_many_val) + ["validation"]*(how_many_val))
        
    dict_to_append = {}
    
    for col, lists in zip(df.columns, [list_samples_image,list_samples_mask,list_samples_seg,list_partition]):
        dict_to_append[col] = lists
    
    df = df.append(pd.DataFrame(dict_to_append))

df.to_csv("dataframe_interiornet_lama_seg.csv",index=False)