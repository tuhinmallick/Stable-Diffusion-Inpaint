...
def process_images(src_images, src_seg_file, indir, outdir, config):
    ...
    for infile, infile_seg in zip(src_images,src_seg_file):
        try:
            ...
            # seg mask
            file_relpath_seg = infile_seg[len(indir):]
            img_outpath_seg = os.path.join(outdir, file_relpath_seg)
            os.makedirs(os.path.dirname(img_outpath_seg), exist_ok=True) # should be useless

            image_seg = Image.open(infile_seg).convert('RGB')
            ...
            if min(image.size) < config.cropping.out_min_size:
                ...
                if handle_small_mode == SmallMode.DROP:
                    ...
                    image_seg = image_seg.resize(out_size, resample=Image.BICUBIC) # should have same size
            else:
                factor = config.cropping.out_min_size / min(image.size)
                out_size = (np.array(image.size) * factor).round().astype('uint32')
                image = image.resize(out_size, resample=Image.BICUBIC)
                image_seg = image_seg.resize(out_size, resample=Image.BICUBIC) 

            # generate and select masks
            src_masks = mask_generator.get_masks(image) # same mask for both seg and normal image
            ...
            for cur_mask in src_masks:
                if config.cropping.out_square_crop:
                    ...
                    cur_image_seg = image_seg.copy().crop((crop_left, crop_top, crop_right, crop_bottom))
                else:
                    cur_image = image
                    cur_image_seg = image_seg

                if len(np.unique(cur_mask)) == 0 or cur_mask.mean() > max_tamper_area:
                    continue

                filtered_image_mask_pairs.append((cur_image, cur_mask, cur_image_seg))
            ...
            for i, idx in enumerate(mask_indices):
                cur_image, cur_mask, cur_seg = filtered_image_mask_pairs[idx]
                ...
                cur_seg.save(cur_basename + f'_seg{i:03d}.png')
...
def main(args):
    ...
    inseg_files = list(glob.glob(os.path.join(args.indir, '**', f'*{args.seg_postfix}.{args.ext}'), recursive=True))
    in_files = [x.replace(args.seg_postfix,"") for x in inseg_files]

    if args.n_jobs == 0:
        process_images(in_files, inseg_files, args.indir, args.outdir, config)
    else:
...
if __name__ == '__main__':
    ...
    aparser.add_argument('--seg_postfix', type=str, help='Path postfix to recognize segmentation mask')
