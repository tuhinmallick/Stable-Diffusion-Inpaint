...
from ldm.models.seginpaint.ddim_seg import DDIMSEGSampler
...
class LatentInpaintDiffusionSEG_CHANNELS(LatentDiffusion):
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=50, ddim_eta=0,
        ...
        for k in self.concat_keys:
            log[k] = rearrange(batch[k], 'b h w c -> b c h w').to(memory_format=torch.contiguous_format).float()
...
class LatentInpaintDiffusionSEG_CHANNELS_RECONSTRUCT(LatentDiffusion):
    """
    can just run as pure inpainting model (only concat mode).
    To disable finetuning mode, set finetune_keys to None
     """
    def __init__(self,
                # DEFAULT FINETUNE KEYS --> use to add novel channels to the concatenation
                 ...
                 concat_keys=("masked_image","seg_mask"), # AS IN INFERENCE
                 masked_image_key="masked_image",
                #  keep_finetune_dims=4,  # if model was trained without concat mode before and we would like to keep these channels
                 keep_finetune_dims=7,  # if model was trained with concat mode before and we would like to keep these channels and add more channels to condition
                 c_concat_log_start=0, # to log reconstruction of c_concat codes, first three channels in our case
    ...
    def get_input(self, batch, k, cond_key=None, bs=None, return_first_stage_outputs=False):
        ...
        if return_first_stage_outputs:
            return z, all_conds, x, xrec, xc
        return z, target_seg, all_conds
    
    def shared_step(self, batch, **kwargs):
        x, target_seg, c = self.get_input(batch, self.first_stage_key)
        loss = self(x, target_seg, c)
        return loss

    def forward(self, x, target_seg, c, *args, **kwargs):
        ...
        return self.p_losses(x, target_seg, c, t, *args, **kwargs)


    def p_losses(self, x_start, target_seg, cond, t, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output = self.apply_model(x_noisy, t, cond)
        # image, segmentation_mask = torch.split(model_output, 2, dim=1)
        loss_dict = {}
        ...
        if self.parameterization == "eps":
            target = torch.cat([noise,target_seg], dim=1)
        else:
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=50, ddim_eta=0,
        ...
        if sample:
            # get denoise row
            with ema_scope("Sampling"):
                samples, seg_mask_x, z_denoise_row = self.sample_log(cond=c_cat,
                                                            batch_size=N, ddim=use_ddim,
                                                            ddim_steps=ddim_steps, eta=ddim_eta)
            ...
            log["samples"] = x_samples
            #seg_mask_x = rearrange(seg_mask_x, 'b h w c -> b c h w').contiguous()
            log["samples_seg_mask"] = seg_mask_x
    ...
    def sample_log(self,cond,batch_size,ddim, ddim_steps, custom = False, **kwargs):
        ...
        if ddim:
            ddim_sampler = DDIMSEGSampler(self)
            # with torch.no_grad():
                # with self.ema_scope():
            # if not custom:
            shape = (self.channels, self.image_size, self.image_size)
            samples, seg_mask_x, intermediates = ddim_sampler.sample(ddim_steps,batch_size,
        ...
        return samples, seg_mask_x, intermediates
